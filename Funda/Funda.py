...
ad = 'Sponsored by Raid Shadow Legends'
        ...
        while j < len(kenmerkTags):
            ...
            if("object-kenmerken-group-header" in tag.get_attribute("class")): # Handle the damned additional lists like "Gebruiksoppervlakten" 
                ...
                for count in range(len(indentTags)):
                    ...
                    indentDict.update({iTag.text:iValue.text.replace("Wat betekent dit?","")})
                    
                output.update({tag.text:indentDict})
            else:
                output.update({tag.text:value.text.replace("Wat betekent dit?","")})
            j +=1
    ...
    def run(self):
        ...
        while len(adLinks) > 0:
            ...
            try:
                ...
                # Print some debug info and restart the browser.
                print(e)
